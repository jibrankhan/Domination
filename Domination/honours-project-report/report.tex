...
When these occur a distinct event object is fired which contains data pertaining to the event. For example when a battle is concluded between two players two events are fired, in the case of a defender retaining their territory it would be a \textit{FailedDefence} event for the defender and a \textit{SuccessfulAttack} event for the attacker. 

These events are observed by the processing class which then routes them to the plan recognition agent. The plan recognition agent then looks at the received event objects and depending on the type of event object handles them appropriately. The types of events are:
