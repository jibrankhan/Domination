...
During the initialisation of the map, a set of explanations must be built for the environment. This is done by the following operation: 
...
Given the consistent actions of each explanation and a list of the territories a player owns one can create a set of actions of any type that a player can perform as well as a sub-set of those actions are consistent or inconsistent with any explanation. To do this is as follows:
...
\begin{pseudocode}[ruled]{ComputeBaseWeight}{weight, }
\begin{algorithm}[H]
\textit{playerPendingSet} = $\emptyset$
