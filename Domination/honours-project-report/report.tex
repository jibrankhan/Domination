...
I would especially like to thank and dedicate this project to my mother and father for going above and beyond, all my life. Without you both; nothing; not a single thing would be possible.
...
Michael Wolf paper calculating state space for RISK is infinite.

Instead of a brute force method of finding the best alternative the idea of plan recognition offers another method. Use plan recognition algorithms we can prune unlikely explanations before committing to a search of the 
...
DID NOT REMOVE SUB-GOAL TERM JUST SET TO 1 EFFECTIVELY REMOVING IT

PHATT computes \textit{Pr(exp} $\wedge$ \textit{obs)} by multiplying three terms together. Choosing to operate on a fully enumerated set of root-goals removes the method choice effectively removes the term \textit{Pr}(\textit{plans}|\textit{goals}), the probability of choosing a sub-goal, from the computation of \textit{Pr(exp} $\wedge$ \textit{obs)}. This modelling choice is thus a design modification to the PHATT algorithm and so the plan recognition agent.

By hard coding every root-goal as explanations of a players behaviour; then assigning that full set of explanations to each player from the start of the game, provided that action probabilities are computed, multiplied with each explanation then stored immediately as soon as they occur. There is no need to store a history of pending sets, to save a list of actions a player has taken or to create and calculate new instances of explanations. This method is similar to that of the 'Test and Generate' algorithm proposed in PHATTs paper rather than the dynamic algorithm proposed at the end of the paper.
...
\hline 
SuccessfulOccupation & Attacking and occupying the territory.\\ 
\hline 
	...
	\ForAll {E $\in$ ES}{
		...
		\If{C \textit{isSubGoalOf} E}{
			...
			\ForAll{T $\in$ C}{
				...
				\textit{addAction}{SuccessfulOccupationT} to \textit{ECA}
...
\item
When player $p_1$ attacks it is always a SuccessfulOccupation action.
\item
...
Figure 3.1 details the R.I.S.K plan library that we will be considering for this example. In this environment there are three root-goals:
...
Each of the three root-goals have several sub-goals. This implies for a player to accomplish that root-goal they must have accomplished each sub-goal and in the context of R.I.S.K means to accomplish occupying continent sub-gaols by performing SuccessfulOccupation actions on each of the several territories contained in that continent. 
...
As we know mission cards are handed out to players' at random, so the prior probability of each root-goal is 1/\textit{n} where \textit{n} is the number of mission cards. Therefore the term \textit{Pr}(\textit{goals}) for each explanation is 1/3. The term \textit{Pr}(\textit{plans}|\textit{goals}) is not taken in account due to previously mentioned design decisions.

Player initially $p_1$ already occupies the territory North-Africa and thus can perform actions connected to the North-Africa square. $p_1$ then achieves the sub-goal of O-Western-Europe through a SuccessfulOccupation action on the territory Western-Europe.

At this point we calculate the term \textit{Pr}(\textit{obs}|\textit{exp}), the probability that $p_1$ would choose to perform the observation O-Egypt given each explanation.

\textit{Pr}(\textit{O-Western-Europe}|\textit{O-AS-AF})

\textit{Pr}(\textit{O-Western-Europe}|\textit{O-EU-SA+1})

\textit{Pr}(\textit{O-Western-Europe}|\textit{O-AS-SA})

To do so requires a $computeExpProb$ operation for each.

First we utilize the $generatePS$ operation to build the full pending set for $p_1$ which contains actions of several different types.

Then we retrieve a list of the consistent actions for each respective explanation by retrieving each explanations $ECA$ from its data structure.

Given the observation O-Egypt, the complete pending set of the player, and the set $ECA$ we use the $computeObsProb$ operation to compute the probability of the observation.

The operation first filters out all actions except SuccessfulOccupation actions from both $p_1$ pending set and each explanations $ECA$. The result is the all the SuccessfulOccupation actions that are consistent with the explanation $expConAct$ and a new pending set $pTotalActNum$ which contains the following actions:
...
Given these two sets we count the number of consistent actions that the players pending set contains for each respective explanation, by comparing what actions are contained in $pTotalActNum$ to each explanations $expConAct$ set.

The counts given the action O-Western-Europe in this example is as follows:

Counts

O-AS-AF

Consistent Actions = 0

O-EU-SA+1

Consistent Actions = 1

O-AS-SA

Consistent Actions = 0

The total actions a player could perform at this time was 5

With these counts we then can compute a $base$ probability for each explanation with the $computeBaseWeight$ operation.

Given our assumed $w$ action probabilities are calculated as follows:
...
Finally we compute \textit{Pr(exp} $\wedge$ \textit{obs)} by multiplying it with one of the two action probabilities based on whether the action performed was consistent or not, in other words if is contained in $ECA$ or not. If so it would be multiplied by $Pr(cons)$ if not $Pr(incons)$

O-AS-AF

\textit{Pr(exp} $\wedge$ \textit{obs)} = 1/3 * 0.2
...
Now another player $p-2$ attacks $p_1$! $p_1$ performs a SuccessfulDefence retaining the territory.

Incorporating these events is much simpler as probabilities are scaled down or up. Given the operations to compute the action probabilities for SuccessfulOccupation.
...
\hline 
SuccessfulOccupation & attackerName, continentName, territoryName & Signals that the player named \textit{attackerName}  performed a SuccessfulOccupation. \\ 
\hline
...
\item
The prediction accuracy of the plan recognition agent is higher for players who win than players who lose.
\item
The difficulty of mission affects the prediction accuracy of the plan recognition agent. IF CANNOT PROVE REMOVE
\item
...
Given there are six mission cards, random guess of a players mission card returns an accuracy of 16.67\%. According to Table 7.1, at its worse the plan recognition agent performs 8.47\% better than random guess and at its best 63.72\% better. 

Table 7.1 reveals that \textit{constrained play has a significantly higher accuracy than the other types of play}. This is unsurprising due to the artificial nature of constrained play. Players only performed directly-consistent actions whereas in Free and A.I. play, players did not. Constrained play was designed to be ideal for correctly predicting explanations and therefore is in actuality, solely not a good measure to evaluate the success of the plan recognition agent, though this does not imply that information gleaned from constrained play cannot be used in evaluation.
...
Though to complete the answer to our initial hypothesis we must then ask, why then did it not successfully converge on the correct explanation 50.72\% of the time? 

Before exploring the possible reasons, the format for the convergence of the plan recognition agent explanation predictions, whether successful or not is as follows.
...
\subsubsection{Winner-Loser Accuracy Differences}

By definition in cases where a player spends all of their game fighting over continents that are non-mission continents the plan recognition agent cannot produce an accurate prediction, as a player was not following or could not follow their mission. By establishing this as an extreme case, if we view the degree of fighting over mission and non-mission continents as a scale, the higher the proportion of fighting over mission continents to fighting over non-mission continents the higher the likelihood of the prediction agent is likely to be correct. NEED TO PROVE CLAIM?

\textit{Winners} are player who have successfully fought to occupy their mission continents, \textit{losers} are players who have failed to do so. In table 7.2 evidence shows that in every type of play \textit{the prediction accuracy of winners has shown to be better than that of losers}.

Present typically examples where this is the case.

This is due to the differences in how the game played out for each player their \textit{game-scenarios}. 

The main game-scenarios that the plan recognition agent faces are the following:

Winners who won \textit{quickly} and \textit{slowly}, who spent the majority of their game:
...
Losers who were eliminated \textit{quickly}, \textit{slowly} and who were \textit{not-eliminated}, who spent the majority of their game:
...
Using this tool allows for the classification of players into their different game-scenarios. As there can be only one winner, the winner row only ever has one entry, whereas the loser row can have 1 to 5 entries.
...
Generally winners and losers were in situations where they either won or lost slowly and the majority of the time they fought over mission continents the plan recognition returned a better accuracy.
...
DO TABLE OF CORRECT EXPLANATIONS OVER PERCENTAGE OF GAME

FOR EACH GAME TYPE

SHORT 25, 50, 75 percent of game
LONG 25, 50, 75 percent of game
...
\subsection{Player-A.I. Analysis}
...
\caption{General Type of Play Accuracy}
...
\hline 
\textbf{Type of Play} & \textbf{Player Type} & \textbf{Correct} & \textbf{Incorrect} & \textbf{\% Accuracy} \\ 
\hline 
\multirow{2}{*}{Free} & Winner & 12 & 8 & 60.00 \\ 
& Loser & 22 & 27 & 44.90 \\ 
\hline 
\multirow{2}{*}{Constrained} & Winner & 15 & 2 & 82.24 \\ 
& Loser & 67 & 18 & 78.82 \\ 
\hline
\multirow{2}{*}{A.I. - Weight 100} & Winner & 80 & 104 & 43.48 \\ 
& Loser & 310 & 610 & 33.70 \\  
\hline
\multirow{2}{*}{A.I. - Weight 4} & Winner & 16 & 43 & 27.12 \\ 
& Loser & 73 & 222 & 24.75 \\ 
\hline
\end{tabular}
}
...
\hline 
Occupy-Europe-Australia-One & 6 & 8 & 42.86 \\  
\hline 
Occupy-Asia-South-America & 7 & 3 & 70.00 \\ 
\hline
Occupy-Europe-South-America-One & 8 & 9 & 47.06 \\
\hline
Occupy-North-America-Africa & 5 & 6 & 45.45 \\
\hline
Occupy-Asia-Africa & 4 & 5 & 44.44 \\
\hline
...
\hline
Occupy-Europe-South-America-One & 0& 1 & 0.00 \\
\hline
...
\textbf{Mission} & \textbf{Correct} & \textbf{Incorrect} & \textbf{\% Accuracy} \\ 
\hline 
Occupy-Europe-Australia-One & 3 & 6 & 33.33 \\  
\hline 
Occupy-Asia-South-America & 5 & 2 & 71.43 \\ 
\hline
Occupy-Europe-South-America-One & 8 & 8 & 50.00 \\
\hline
Occupy-North-America-Africa & 1 & 5 & 16.67 \\
\hline
Occupy-Asia-Africa & 3 & 4 & 42.86 \\
\hline
Occupy-North-America-Australia & 2 & 2 & 50.00 \\
\hline
\end{tabular}
...
\section{Game Length Correct-Incorrect Prediction Counts}

\subsection{General Game-Length Correct-Incorrect Prediction Count}
...
\caption{Constrained Game-Length Correct-Incorrect Prediction Count, \textbf{C} = Correct, \textbf{I} = Incorrect}
\label{table:winner-loser-accuracy}
\end{table}
