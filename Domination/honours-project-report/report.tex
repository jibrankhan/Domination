...
Built purely in the Java programming language, the plan recognition agent follows an \textit{event-driven} system architecture, with the following design.
...
\textit{Event} objects contain important data from the environment which the plan recognition agent requires to compute the likelihood of explanations. These events are fired from the \textit{Game} at the following key points:
...
MORE DETAIL OF SPECIFIC IMPLEMENTATION REQUIRED
...
In this way the implementation of the plan recognition agent is not as a dependant sub-component of the game but rather a component completely separate entity that operates in \textit{parallel} to the game.
