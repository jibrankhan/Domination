...
\hline 
New-Player & playerName & Initialises a new player data structure with ID \textit{playerName}. \\ 
\hline 
Remove-Player & playerName & Destroys the player data structure with ID \textit{playerName}. \\ 
\hline 
Successful-Occupation & attackerName, cName, tName & Signals that the player named \textit{attackerName}  performed a Successful-Occupation. \\ 
\hline
Failed-Occupation & attackerName, cName, tName & Signals that \textit{attackerName} performed a FailedOccupation. \\ 
\hline
Successful-Defence & defenderName, cName, tName & Signals that \textit{defenderName} performed a SuccesfulDefence. \\ 
\hline
Failed-Defence & defenderName, cName, tName & Signals that \textit{defenderName} performed a FailedDefence. \\ 
\hline
Army-Movement & playerName, cName, tName & Signals that \textit{playerName} performed an ArmyMovement. \\ 
\hline
Reinforce & playerName, cName, tName & Signals that \textit{playerName} performed a Reinforce action. \\ 
\hline
\end{tabular}
}
...
More specifically other than player initalisation or removal events, each event will trigger the computation of explanation probabilites, which players explanation probabilites are decieded for example by the \textit{attackerName} and what effect, is decieded by the state of the R.I.S.K world namely the respective players pending set which can be built from the data structures of the Domination-Program and the continent and territory name of the event.

\section{System Operation}

Before a game begins, based on the players registered in the game lobby screen the plan recognition agent initialises a set of player data strucutres. 
...
Looking at an example of constrained play.
...
The human players mission was North-America Africa, for the majority of the game during the placement phase up until turn 82 the probability does not rise too much. Though after turn 82 when the attack phases begin the increase in explanaton probability is much more significant and convergence for this example to the correct explanation occurs within 10 turns, reaching a significantly higher explanation probability soon after attacks have begun. 
...
This graph illustrates the probabilites of explanations of an A.I. 4 player whos misson was Asia South-America. The A.I won a game that was approximately 342 turns long. In this game the A.I.s were caught in a stalemate between each other for many turns, argueably though not out of skill but instead a lack of strategic diversity. For example A.I. players make no attempt to fortify and hold continents when they are weak or to consolidate their armies if they can be concentrated in strategic territories. Instead it would go for a strategy of an all-out attack. Mostly fighting in North-America and Australia, continents other than its mission.

The A.I. at one point had a clear advantage but chose to attack a non-mission continent to eliminate a weaker player first.  Asia its final mission continent was conquered at the very last turn and so ending the game.
...
There are cases where winners have occupied the an entire continent and completed their mission in one turn and since sampling is done on a turn by turn basis the plan recognition agent will miss the significant change in probabilities as the system does not register an end of turn but rather a different event which is the end of the game.
...
Losers are often prevented from successfully achieving their goal as they may be eliminated early or may be pushed out of their mission continents. The following is an example of a player with mission North-America Africa being eliminated from a game at approximately turn 75. 
...
A proportion of incorrect predictions occured because losers can be eliminated at an early stage of the game resulting in a lack of data about that player. If a loser was to be eliminated early on in the game then their explanations probabilities would \textit{flat-line} as we can clearly see from Figure 5.10, in that the prediction does not change for the remainder of the game. If the loser had been performing more inconsistent actions then consistent actions up until that point then the prediction will be wrong and will remain an incorrect plan prediction, inversely it may be correct for the very same reason.

Unlike the significant fall seen in A.I. -  4 Winners the plan prediction accuracy of A.I - 4 Losers remains stable.
